...
def fix_data_types(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'sale_date' in df_fixed.columns:
        if not pd.api.types.is_datetime64_any_dtype(df_fixed['sale_date']):
            df_fixed['sale_date'] = pd.to_datetime(df_fixed['sale_date'], errors='coerce')
    
    # Ensure numerical columns are actually numeric
    numeric_columns = ['price', 'square_feet', 'lot_size', 'age', 'bedrooms', 'bathrooms', 'garage']
    ...
    for column, method in strategy.items():
        if column in df_imputed.columns and df_imputed[column].isnull().any():
            ...
            if method == 'drop':
                ...
                print(f"Filled {missing_count} missing {column} values with mean: {fill_value:.2f}")
                
            elif method == 'median':
                fill_value = df_imputed[column].median()
                df_imputed[column] = df_imputed[column].fillna(fill_value)
                print(f"Filled {missing_count} missing {column} values with median: {fill_value}")
                ...
                if len(df_imputed[column].mode()) > 0:
                    ...
                    fill_value = 'Unknown'
                df_imputed[column] = df_imputed[column].fillna(fill_value)
                print(f"Filled {missing_count} missing {column} values with mode: {fill_value}")
                ...
                fill_value = 0 if df_imputed[column].dtype in ['int64', 'float64'] else False
                df_imputed[column] = df_imputed[column].fillna(fill_value)
                print(f"Filled {missing_count} missing {column} values with constant: {fill_value}")
...
def validate_data(df: pd.DataFrame) -> Dict[str, any]:
    ...
    if 'price' in df.columns:
        ...
        if cheap_houses > 0:
            validation_results['issues'].append(f"{cheap_houses} houses with price < $10,000")
        if expensive_houses > 0:
            validation_results['issues'].append(f"{expensive_houses} houses with price > $10M")
    
    if 'square_feet' in df.columns:
        tiny_houses = (df['square_feet'] < 100).sum()
        huge_houses = (df['square_feet'] > 20000).sum()
        if tiny_houses > 0:
            validation_results['issues'].append(f"{tiny_houses} houses with < 100 sq ft")
        if huge_houses > 0:
            validation_results['issues'].append(f"{huge_houses} houses with > 20,000 sq ft")
...
def preprocess_housing_data(df: pd.DataFrame, 
    ...
    print(f"Final dataset: {validation['total_records']} records, {validation['total_columns']} columns")
