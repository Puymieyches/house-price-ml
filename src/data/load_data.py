...
def add_missing_data_patterns(df: pd.DataFrame) -> None:
    """
    Add realistic missing data patterns to the housing dataset.

    Args:
        df: DataFrame to modify in-place
    """

    logger.info("Adding realistic missing data patterns...")

    # 1. Missing lot_size for condos (common in real estate data)
    condo_mask = df["property_type"] == "Condo"
    condo_indices = df[condo_mask].index.tolist()

    if len(condo_indices) > 0:
        n_missing = int(len(condo_indices) * 0.8)  # 80% of condos
        if n_missing > 0:
            missing_indices = np.random.choice(condo_indices, n_missing, replace=False)
            df.loc[missing_indices, "lot_size"] = np.nan
            logger.info("Set lot_size to NaN for %d condos", n_missing)

    # 2. Some missing garage info (2% of all properties)
    all_indices = df.index.tolist()
    n_missing_garage = int(len(all_indices) * 0.02)
    if n_missing_garage > 0:
        missing_garage_indices = np.random.choice(
            all_indices, n_missing_garage, replace=False
        )
        df.loc[missing_garage_indices, "garage"] = np.nan
        logger.info("Set garage to NaN for %d properties", n_missing_garage)

    # 3. Some missing condition ratings (5% of all properties)
    n_missing_condition = int(len(all_indices) * 0.05)
    if n_missing_condition > 0:
        missing_condition_indices = np.random.choice(
            all_indices, n_missing_condition, replace=False
        )
        df.loc[missing_condition_indices, "condition"] = np.nan
        logger.info("Set condition to NaN for %d properties", n_missing_condition)
...
def generate_realistic_housing_data(n_samples: int = 10000) -> pd.DataFrame:
    ...
    # Add realistic missing data patterns (FIXED VERSION)
    add_missing_data_patterns(df)

    # Add some outliers (data entry errors)
    logger.info("Adding realistic outliers...")
    outlier_indices = np.random.choice(
        len(df), size=int(len(df) * 0.001), replace=False
    )
    for idx in outlier_indices:
        if np.random.random() < 0.5:
            # Price outlier
            df.loc[idx, "price"] *= np.random.choice(
                [0.1, 10]
            )  # Very cheap or very expensive
        else:
            # Size outlier
            df.loc[idx, "square_feet"] *= np.random.choice(
                [0.1, 5]
            )  # Very small or very large

    logger.info("Generated dataset with shape: %s", df.shape)
    return df
